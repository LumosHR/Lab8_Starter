describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const addBtn = await shadowRoot.$('button');
    await addBtn.click();
    const btnInnerText = await addBtn.getProperty('innerText');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const sizeOfCart = await page.$('#cart-count');
    const cartInnerText = await sizeOfCart.getProperty('innerText');
    expect(cartInnerText['_remoteObject'].value).toBe("20");
  }, 10000);
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const storedData = await page.evaluate(() => JSON.parse(window.localStorage.getItem('cart')));
    for (let i = 0; i < 20; i++) {
      expect(storedData[i]).toBe(i + 1);
    }
  });
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const sizeOfCart = await page.$('#cart-count');
    const cartInnerText = await sizeOfCart.getProperty('innerText');
    expect(cartInnerText['_remoteObject'].value).toBe("0");
  }, 10000);
